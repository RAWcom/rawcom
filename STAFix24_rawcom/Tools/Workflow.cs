...
namespace Tools
{
    public class Workflows
    {
        ...
        public static SPWorkflow StartSiteWorkflow(SPSite site, string workflowName)
        {
            using (SPWeb web = site.OpenWeb()) // get the web
            {
                ...
                return site.WorkflowManager.StartWorkflow(null, assoc, string.Empty, SPWorkflowRunOptions.Synchronous);
        ...
        public static void AssociateWorflow(SPWeb web, string workflowTemplateBaseGuid, string workflowAssociationName)
        {
            ...
            if (workflowTemplate != null)
            {
                ...
                if (taskList == null)
                {
                    taskList = Workflows.CreateTaskList(web, workFlowTaskListName);
                }
                SPList historyList = web.Lists.TryGetList(workFlowHistoryListName);
                if (historyList == null)
                {
                    historyList = Workflows.CreateHistoryListy(web, workFlowHistoryListName);
                }
